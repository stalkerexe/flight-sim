...
export interface ChunkCoordPlain {
  readonly cx: number;
  readonly cz: number;
  readonly lod: number;
}

/** Сырые типизированные массивы геометрии — то, что можно передать между потоками через Transferable. */
...
export function resolutionForLod(lod: number): number {
 ...
 */
export function chunkSizeForLod(lod: number): number {
...
export function buildChunkArrays(
  ...
  const copyDown = (mainIdx: number, skirtIdx: number): void => {
    const mi = mainIdx * 3;
    const si = skirtIdx * 3;
...
function biomeColor(height: number): [number, number, number] {
  if (height < WORLD.SEA_LEVEL) return [0.09, 0.24, 0.32];
  if (height < WORLD.SEA_LEVEL + 15) return [0.62, 0.58, 0.4];
  if (height < WORLD.MAX_TERRAIN_HEIGHT * 0.35) return [0.18, 0.32, 0.14];
  if (height < WORLD.MAX_TERRAIN_HEIGHT * 0.7) return [0.32, 0.28, 0.22];
