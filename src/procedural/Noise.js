...
export class TerrainNoise {
     ...
     */
    getHeight(worldX, worldZ, maxFrequency = Infinity) {
        const continental = this.continentNoise(worldX * NOISE.CONTINENT_FREQUENCY, worldZ * NOISE.CONTINENT_FREQUENCY);
        const island = this.islandNoise(worldX * NOISE.ISLAND_FREQUENCY, worldZ * NOISE.ISLAND_FREQUENCY);
        ...
        const warpX = this.warpNoiseX(worldX * NOISE.WARP_FREQUENCY, worldZ * NOISE.WARP_FREQUENCY) *
            NOISE.WARP_STRENGTH;
        const warpZ = this.warpNoiseZ(worldX * NOISE.WARP_FREQUENCY, worldZ * NOISE.WARP_FREQUENCY) *
            NOISE.WARP_STRENGTH;
        ...
        const mountainRaw = this.mountainMaskNoise(worldX * NOISE.MOUNTAIN_MASK_FREQUENCY, worldZ * NOISE.MOUNTAIN_MASK_FREQUENCY);
        ...
        const amplitude = NOISE.FLAT_AMPLITUDE * (1 - mountainMask) + NOISE.MOUNTAIN_AMPLITUDE * mountainMask;
    ...
    fbm(x, z, maxFrequency) {
        ...
        for (let octave = 0; octave < NOISE.OCTAVES; octave++) {
            if (frequency > maxFrequency)
                break; // дальше только aliasing — не считаем
