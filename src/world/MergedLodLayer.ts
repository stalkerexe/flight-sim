...
export class MergedLodLayer {
  ...
  private readonly chunks = new Map<string, ChunkArrays>();
  private dirty = false;

  constructor(lod: number, material: THREE.Material, parent: THREE.Group) {
    this.lod = lod;
  ...
  setChunk(key: string, arrays: ChunkArrays): void {
  ...
  pruneToDesired(desiredKeys: ReadonlySet<string>): boolean {
    let changed = false;
  ...
  rebuildIfDirty(): void {
    ...
    for (const g of geometries) g.dispose(); // временные геометрии больше не нужны — данные уже скопированы в merged
  ...
  updatePosition(origin: Readonly<{ x: number; y: number; z: number }>): void {
    if (!this.mesh) return;
