...
import { TerrainChunk, chunkKey } from './TerrainChunk';
...
export class ChunkManager {
    ...
    active = new Map();
    mergedLayers = new Map();
    dispatchQueue = [];
    inFlight = new Set();
    desiredKeys = new Set();
    lastAnchorCellX = Number.NaN;
    lastAnchorCellZ = Number.NaN;
    lastAnchorWorldX = 0;
    lastAnchorWorldZ = 0;
    /** Текущие радиусы LOD-колец в метрах — меняются QualityManager'ом в рантайме. */
    ringRadiusMeters;
    lastMergeRebuildAt = new Map();
    ...
    static MERGE_REBUILD_STALE_MS = 400;
    constructor(parentScene, floatingOrigin, initialRingRadiusMeters) {
    ...
    rebuildDesiredSet(anchorWorldX, anchorWorldZ) {
            ...
            for (let dz = -outerRadius; dz <= outerRadius; dz++) {
                for (let dx = -outerRadius; dx <= outerRadius; dx++) {
                    ...
                    if (dist > innerRadius && dist <= outerRadius) {
                        const coord = { cx: anchorChunkX + dx, cz: anchorChunkZ + dz, lod };
    ...
    processDispatchQueue() {
        ...
        while (dispatched < WORKER_POOL.MAX_DISPATCH_PER_FRAME && this.dispatchQueue.length > 0) {
            ...
            this.workerPool
                ...
                if (!this.desiredKeys.has(key) || this.isActive(key, coord.lod))
                    return;
                if (coord.lod === 0) {
    ...
    maybeRebuildMergedLayers() {
        const busyLods = new Set();
        for (const coord of this.dispatchQueue)
            busyLods.add(coord.lod);
        for (const key of this.inFlight)
            busyLods.add(lodFromKey(key));
    ...
    positionIndividualChunk(chunk) {
        const origin = this.floatingOrigin.getOrigin();
        chunk.mesh.position.set(chunk.worldOriginX - origin.x, -origin.y, chunk.worldOriginZ - origin.z);
...
function worldDistToChunkCenter(coord, worldX, worldZ) {
