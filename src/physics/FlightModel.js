 ...
 */
export function computeAerodynamics(velocity, orientation, throttle) {
    ...
    const controlEffectiveness = Math.min((airspeed * airspeed) / (FLIGHT.CONTROL_EFFECTIVENESS_REF_SPEED * FLIGHT.CONTROL_EFFECTIVENESS_REF_SPEED), 1);
...
function computeLiftCoefficient(aoaRad, stallAngleRad) {
...
function computeDragCoefficient(cl, aoaRad, stallAngleRad) {
